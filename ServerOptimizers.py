...
class FedAvgAdaptive:
    ...
    def __init__(self,model:list,n_clients=2,lr=1e-3,beta_2=0.999,eps = 1e-8,q=5,weights = None):
    ...
    def aggregate_and_update(self, grads=None):
        ...
        if self.update_count % self.q != 0:
            ...
            if self.weights is None:
                ...
                grads = np.array([itm*self.weights[i] for i,itm in enumerate(grads)])
            av_x = sum(self.x) / len(self.x)
            for idx in range(len(self.client_vars)):
                self.x[idx] = av_x + self.lr*grads[idx]/(np.sqrt(self.client_vars[idx])+self.eps)
                self.mes[idx].set_flattened_params_all(self.x[idx])
        self.update_count += 1
...
class FAFED:
    ...
    def init_states(self):
        self.x = [me.get_flattened_params() for me in self.mes]
        self.m = [np.zeros((self.num_params,)) for _ in range(self.n_clients)]
        self.v = [np.ones((self.num_params,)) for _ in range(self.n_clients)]
    ...
    def aggregate_and_update(self, grads=None):
        ...
        if self.update_count % self.q != 0:
            ...
            if self.weights is None:
                ...
                grads = np.array([itm*self.weights[i] for i,itm in enumerate(grads)])
            av_m,av_v = sum(self.m) / len(self.m), sum(self.v) / len(self.v)
            av_x = sum(self.x) / len(self.x)
            for idx in range(self.n_clients):
                self.m[idx], self.v[idx] = av_m, av_v
                self.x[idx] = av_x + self.lr*grads[idx]/(np.sqrt(self.v[idx])+self.eps)
                self.mes[idx].set_flattened_params_all(self.x[idx])
        self.update_count += 1
